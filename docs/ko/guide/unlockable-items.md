...
The unlockable state of the items is stored under [global scope](/ko/guide/state-management#global-state) and doesn't depend on local game sessions; eg, if you unlock some item, it won't become locked again when player starts a new game or loads another saved game.
...
In C# you can access the unlockable items using `UnlockableManager` [engine service](/ko/guide/engine-services).
...
The unlockable resources are used by the built-in unlockable systems, such as [CG Gallery](/ko/guide/unlockable-items#cg-gallery). You can also utilize the manager for you own custom systems.
...
By default, all the unlockable texture resources with `CG` prefix added via [unlockable resources manager](/ko/guide/unlockable-items#unlockable-resources) and [background](/ko/guide/backgrounds) sprite resources of the `MainBackground` actor with the same prefix will be considered unlockable CG items.
...
You can modify or completely replace the built-in `ICGGalleryUI` implementation using the [UI customization feature](/ko/guide/user-interface#ui-customization).

## Tips

Unlockable tips system allows to specify a set of text records using localizable [managed text](/ko/guide/managed-text) documents; the records can then be unlocked throughout the game and be browsed via the `ITipsUI` UI accessible from the title menu and text printer control panels.
...
It's possible to automatically unlock tips when associated text is printed via a [TMPro printer](/ko/guide/text-printers#textmesh-pro); additionally, when such text is clicked by player `ITipsUI` UI will be automatically shown with the associated tip record selected.
