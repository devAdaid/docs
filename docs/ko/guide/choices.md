...
::: info NOTE
Find more about labels in the [scrips guide](/ko/guide/naninovel-scripts#label-lines).
:::
...
Choice handlers behavior can be configured using `Naninovel -> Configuration -> Choice Handlers` context menu; for available options see [configuration guide](/ko/guide/configuration#choice-handlers).
...
If you don't want to store the choice button prefabs in `Resources` folder or need to localize them, set a custom loader in choice handler configuration menu and use any of the available [resource providers](/ko/guide/resource-providers).
...
To change choice button used by default, create a [custom choice handler](/ko/guide/choices#adding-custom-choice-handlers) and assign the prefab to `Default Button Prefab` property of `Choice Handler Panel` component or use a custom component.
...
Used by [chat text printer](/ko/guide/text-printers#chat-printer) to represent reply choices. Example:
...
It's also possible to create a choice handler from scratch by manually implementing `IChoiceHandlerActor` interface. See the guide on [custom actor implementations](/ko/guide/custom-actor-implementations) for more information.
