...
Characters are actors used to represent scene entities that are placed on top of the [backgrounds](/ko/guide/backgrounds).
...
Characters' behavior can be configured using `Naninovel -> Configuration -> Characters` context menu; for available options see [configuration guide](/ko/guide/configuration#characters). The characters' resources manager can be accessed using `Naninovel -> Resources -> Characters` context menu.
...
The content of the curly braces is actually treated as a full-fledged [script expression](/ko/guide/script-expressions), allowing complex scenarios for evaluating the display name. For example, you may want to keep a pre-defined localizable display name for a character until some point and then let the player pick a custom name.

Let's say the character in questions has "Char1" ID, pre-defined name is stored as "T_PredefinedName" [managed text record](/ko/guide/managed-text#script-text), the value entered by the player will be stored as "name" [custom variable](/ko/guide/custom-variables) and "nameSet" variable will be set to "true" when player has set the name. Assign the following expression to the `Display Name` property: `{ nameSet ? name : T_PredefinedName }`.
...
It's possible to associate a [text printer](/ko/guide/text-printers) with a character using `Linked Printer` property.
