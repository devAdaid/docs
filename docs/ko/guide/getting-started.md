...
또 다른 널리 사용되는 개념은 [사용자 인터페이스](/ko/guide/user-interface) (UI) 입니다. UI는 플레이어가 액터 등 게임과 상호작용하는 데에 사용됩니다. UI에는 다양한 메뉴 (타이틀, 저장-불러오기, 설정 등) 및 컨트롤 패널(자동 읽기 모드 전환, 텍스트 스킵 등)이 포함됩니다.
...
프로젝트 생성 시, 대부분의 경우 에디터를 2D 동작 모드로 설정하기 위해 `2D 템플릿`을 선택하게 됩니다. 이렇게 하면 기본적으로 이미지 파일이 스프라이트 어셋으로 임포트되어, 수동으로 임포트 설정을 변경할 필요가 없어지게 됩니다. 이후 [Project Settings](https://docs.unity3d.com/Manual/2DAnd3DModeSettings.html)에서 에디터 동작 모드를 2D/3D로 변경할 수 있습니다.
...
`Naninovel.package` 파일을 Unity 에디터의 project 창에 끌어다 놓습니다. (에셋 스토어에서 Naninovel을 구입한 경우, [Asset Store 메뉴](https://docs.unity3d.com/Manual/AssetStore.html)를 통해 패키지를 임포트합니다.) 이후 초기 스크립트 컴파일 및 어셋 임포트 과정이 끝나기를 기다립니다. 필요한 경우, `Naninovel` 패키지 폴더는 프로젝트 Assets 디렉토리 내에서 자유롭게 이동할 수 있습니다.
...
비주얼 에디터를 사용하여 스크립트를 변경한 경우, Inspector 창 헤더에서 스크립트 이름 위에 별표(`*`)가 표시됩니다. 이는 어셋이 수정되었으며 저장해야 함을 의미합니다. 어셋을 저장하려면 `Ctrl+S`를 누르세요. 스크립트가 수정된 상태에서 다른 에셋을 선택한다면, 수정중인 스크립트의 변경 사항을 저장하거나 취소할 수 있는 대화창이 표시됩니다.
...
Naninovel 관련 어셋 (예시: 위에서 생성한 스크립트)이 엔진에 "표시"되려면, 프로젝트 리소스로 할당되어야 합니다. Create 메뉴를 통해 스크립트를 생성하면, 자동으로 프로젝트 리소스에 할당됩니다. 수동으로 스크립트 리소스를 할당하려면 에디터 상단바에서 `Naninovel -> Resources -> Scripts` 메뉴로 열 수 있는 스크립트 리소스 창을 사용하세요. 스크립트를 추가하려면 먼저 목록에 새 항목을 추가하기 위해 `+` (플러스) 버튼을 클릭하고, 그 다음 스크립트 어셋을 목록 위에 드래그하여 놓으세요. 여러 어셋이나 폴더를 목록에 드래그해 일괄 추가할 수도 있습니다.
...
다른 배경으로 전환할 때는 크로스 페이드 [전환 효과](/ko/guide/transition-effects)가 기본으로 사용됩니다. 전환 효과를 변경하려면, 다음과 같이 외관 이름 다음에 전환 효과를 지정합니다.
...
이 명령은 "City"를 "School"로 "RadialBlur" 전환 효과를 사용하여 전환합니다.
...
BGM (배경음) 또는 SFX (효과음) 어셋을 추가하려면 `Naninovel -> Resources -> Audio` 에디터 메뉴를 사용하세요. [Unity에서 지원하는](https://docs.unity3d.com/Manual/AudioFiles.html) 모든 오디오 형식을 사용할 수 있습니다.
...
동영상 가이드를 선호하는 경우, 위 과정을 설명하는 동영상 가이드 또한 준비되어 있습니다.
