...
For more information on how to use the tool, see [localization guide](/ko/guide/localization).
...
Specify the required folders:
 ...
 - Input Text Folder — folder where [managed text documents](/ko/guide/managed-text) are generated to; it's `Assets/Resources/Naninovel/Text` by default. Make sure to generate managed text documents via associated tool in case the folder is missing.
