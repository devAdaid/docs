...
--- | ---
IBackgroundManager | Manages [background](/ko/guide/backgrounds) actors.
ICharacterManager | Manages [character](/ko/guide/characters) actors.
IChoiceHandlerManager | Manages [choice handler](/ko/guide/choices) actors.
ITextPrinterManager | Manages [text printer](/ko/guide/text-printers) actors.
IAudioManager | Manages the audio: [SFX](/ko/guide/audio#sound-effects), [BGM](/ko/guide/audio#background-music) and [voicing](/ko/guide/voicing).
IInputManager | Manages the user [input processing](/ko/guide/input-processing).
ILocalizationManager| Manages the [localization](/ko/guide/localization) activities.
ITextManager | Handles [managed text](/ko/guide/managed-text) feature.
IMoviePlayer | Handles [movie](/ko/guide/movies) playing.
IScriptManager | Manages [naninovel script](/ko/guide/naninovel-scripts) resources.
IScriptPlayer | Handles [naninovel scripts](/ko/guide/naninovel-scripts) execution.
...
IStateManager | Handles `IEngineService`-related persistent data de-/serialization; provides API to [save and load](/ko/guide/save-load-system) game state.
IUIManager | Manages `IManagedUI` objects and handles [UI customization](/ko/guide/user-interface#ui-customization) feature.
ICustomVariableManager | Provides access and allows modifying [custom variables](/ko/guide/custom-variables).
ISpawnManager | Manages objects spawned with [@spawn] commands.
IUnlockableManager | Manages [unlockable items](/ko/guide/unlockable-items) (CG and movie gallery items, tips, etc).
...
In case your custom service has a persistent state, which you wish to de-/serialize with other engine services, implement `IStatefulService<TState>` interface, where `TState` is either `GameStateMap`, `GlobalStateMap` or `SettingsStateMap` depending if you want to store the state with the game sessions-specific, global state or settings. It's allowed to implement all three interfaces for a single service if required. For more information on different types of engine state see [state management guide](/ko/guide/state-management).
