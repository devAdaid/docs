...
Background music (BGM) and sound effects (SFX) are covered in this article; for the voices see [voicing guide](/ko/guide/voicing).
...
Audio playback behavior can be configured using `Naninovel -> Configuration -> Audio` context menu; for available options see [configuration guide](/ko/guide/configuration#audio).
...
In C# scripts, currently used audio mixer can be retrieved via `IAudioManager` [engine service](/ko/guide/engine-services).
