...
An author ID can be specified at the start of a generic text line separated by a column followed with space ((`: `)) to associate printed text with a [character actor](/ko/guide/characters):
...
All the commands (both [built-in](/api/) and [custom ones](/ko/guide/custom-commands)) can be inlined (injected) to generic text lines using square brackets (`[ ]`):
...
For more information on the conditional expression format and available operators see [script expressions](/ko/guide/script-expressions) guide.
...
In case you don't need any [engine service](/ko/guide/engine-services) state to persist between scripts (including currently played music, special effects, etc), enable `Reset On Goto` in state configuration. This way all the services will automatically dispose associated resources when navigating between naninovel scripts and you won't have to use any special commands to unload the resources.
...
When [comment lines](/ko/guide/naninovel-scripts#comment-lines) are present at the top of a script, associated graph node will show those as a synopsis. To disable that, uncheck `Show Synopsis` in the scripts configuration menu.
...
To manually initiate hot reload of the currently played naninovel script (eg, when editing script file outside of Unity project), use `reload` [console command](/ko/guide/development-console). The command is editor-only (won't work in builds).
...
For more information on how to install and use the extension see the [IDE extension guide](/ko/guide/ide-extension).
...
Using [development console](/ko/guide/development-console) you can instantly "rewind" currently played script to an arbitrary line:
