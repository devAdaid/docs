...
Printers' behavior can be configured using `Naninovel -> Configuration -> Printers` context menu; for available options see [configuration guide](/ko/guide/configuration#printers). The printers' resources manager can be accessed using `Naninovel -> Resources -> Printers` context menu.
...
Consult [built-in UI customization guide](/ko/guide/user-interface#modifying-built-in-ui) for more info now how to customize and configure the UI.
...
Wide printers are very like dialogue printers, except for some changes in the panel layout tailored for wide displays. Wide printers also support [character avatars](/ko/guide/characters#avatar-textures) feature.
...
::: tip EXAMPLE
Check out [demo project](/ko/guide/getting-started#demo-project) for an example on adding a custom printer. The prefab is stored as `Assets/Prefabs/CustomPrinter.prefab`.
:::

It's also possible to create a printer from scratch by manually implementing `ITextPrinterActor` interface. See the guide on [custom actor implementations](/ko/guide/custom-actor-implementations) for more information.
...
In case the reveal sounds are not working for you (eg, the sound is not short enough to play it on each char reveal), consider using `OnPrintTextStarted` and `OnPrintTextFinished` events of the `TextPrinterManager` [engine service](/ko/guide/engine-services) to start/stop looping the sound accordingly. Those events are also exposed to PlayMaker, in case you prefer a [visual scripting](/ko/guide/playmaker) solution.
...
You can use event tags instead of [inlined commands](/ko/guide/naninovel-scripts#command-inlining) to expose them to [localization documents](/ko/guide/localization#scripts-localization) allowing translators override executed commands based on selected locale.
